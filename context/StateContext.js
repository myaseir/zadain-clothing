...
export const StateContext = ({ children }) => {
  ...
  const onAdd = (product, quantity, size) => {
    ...
    if(checkProductInCart) {
      const updatedCartItems = cartItems.map((cartProduct) => {
        if(cartProduct._id === product._id && cartProduct.selectedSize === size) {
          return {
            ...cartProduct,
      ...
      setCartItems([...cartItems, { ...product, quantity, selectedSize: size }]);
    }

    toast.success(`${quantity} ${product.name} (Size: ${size}) added to cart.`);
  } 
  ...
  const toggleCartItemQuantity = (id, size, value) => {
    foundProduct = cartItems.find((item) => 
      item._id === id && item.selectedSize === size
    );
    index = cartItems.findIndex((product) => 
      product._id === id && product.selectedSize === size
    );
    const newCartItems = cartItems.filter((item) => 
      !(item._id === id && item.selectedSize === size)
    );
